...
func _can_drop_data(_at_position: Vector2, data: Variant) -> bool:
	if data is WordVis:
		if data._get_word_vis_word() is Noun and _is_empty() and not(data._is_locked()) and data._get_word_vis_word()._get_word_time_tense() != "past":
			print("You can drop a Noun here")
			return true
		else:
			print("The word you are trying to drop is not a NOUN")
			return false
	else:
...
func _cycle_noun(direction:String)->void:
	...
	var nounInWordVis=wordVisInBay._get_word_vis_word()
	nounInWordVis._cycle_noun(direction)
	wordVisInBay._set_word_vis_word(nounInWordVis)
